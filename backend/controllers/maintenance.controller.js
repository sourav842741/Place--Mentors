...
export const getRandomByType = asyncHandler(async (req, res) => {
  ...
  res.status(200).json(new ApiResponse(200, question[0], `${type} question fetched`));
});
...
export const getDashboardData = asyncHandler(async (req, res) => {
  ...
  const pipeline = types.map((type) => ({
    $facet: {
      [`${type}`]: [{ $match: { type, active: true } }, { $sample: { size: 1 } }],
    },
  }));
...
export const getAllTypes = asyncHandler(async (req, res) => {
  ...
  res.status(200).json(new ApiResponse(200, stats, "Types stats fetched"));
});
...
export const listQuestions = asyncHandler(async (req, res) => {
  const { page = 1, limit = 20, type, search, active } = req.query;
...
export const addQuestion = asyncHandler(async (req, res) => {
  ...
  res.status(201).json(new ApiResponse(201, created, "Question added successfully"));
});
...
export const updateQuestion = asyncHandler(async (req, res) => {
  ...
  res.status(200).json(new ApiResponse(200, updated, "Question updated"));
});
