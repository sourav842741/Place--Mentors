  ...
  const fetchVideo = async (query) => {
    try {
      ...
      const res = await api.get(
        `/api/ai/youtube?query=${encodeURIComponent(query)}`
      );
        ...
        <div className="grid md:grid-cols-2 xl:grid-cols-3 gap-6">
          {filtered.map((item, i) => (
            <Card key={i} className="p-6 rounded-2xl">
              {item.badge && (
                <Badge className="absolute top-4 right-4">{item.badge}</Badge>
              )}
      ...
      >
        <DialogContent className="max-w-4xl rounded-2xl">
          ...
          <DialogHeader>
            ...
            <DialogDescription>
              Watch curated DSA explanation video
            </DialogDescription>
          </DialogHeader>
