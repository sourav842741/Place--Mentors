...
const fetchWithRetry = async (
  ...
  try {
    ...
    if (retries > 0 && isTimeout) {
      ...
      const delay =
        retryDelays[retries] || 60000;

      console.log(
        `Retrying Adzuna API in ${
          delay / 1000
        } seconds... Attempts left: ${retries}`
      );
...
export const fetchAdzunaJobs = async (
  ...
  try {
    ...
    console.log(
      `Done: ${what} in ${where} (${jobs.length})`
    );
    ...
    throw new Error(
      `Failed fetching Adzuna jobs: ${error.message}`
    );
  }
};
...
export const saveJobsToDb = async (jobs) => {
  ...
  for (const jobData of jobs) {
    try {
      ...
      if (error.code === 11000) {
        console.log(
          `Duplicate skipped: ${jobData.title}`
        );
      } else {
        console.error(
          "Save job error:",
